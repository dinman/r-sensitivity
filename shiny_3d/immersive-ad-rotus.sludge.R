...
library(devtools)
install_github("InsightCenter/iplot", host="github.nrel.gov")
load_all("/Users/aeberle/Documents/iplot-master", export_all = TRUE)


iplotAvailable <- require(iplot)


load("../shiny_scratch/sampled_design.rotus.sludge.ratereturn.31may.fixpng.RDA")
load("../shiny_scratch/sampled_results.rotus.sludge.ratereturn.31may.fixpng.RDA")
...
ui <- {
  ...
  fluidPage(
    ...
    titlePanel("Local Sensitivity Analysis of ROTUS Sludge Using Mood"),
...
server <- function(input, output, session) {
  ...
  output$ad.results <- renderTable({
    ...
    withProgress(message = "Calculation in progress \n", value = 0, {
      results <- run.mood (selection=values$ids, design=vbsa.design, boots=5, design.melted=TRUE, alpha=0.8)
